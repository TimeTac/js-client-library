...
describe('TimePlannings', () => {
  ...
  const readPath: string = `${timePlannings.getResourcePath()}/read`;
  ...
  test('read with status code 500', async () => {
    ...
    expect.assertions(1);
    await timePlannings.read().catch((err) => expect(err.message).toMatch('Request failed with status code 500'));
  });
