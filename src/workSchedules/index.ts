import BaseApi from '../baseApi';
import { Entity } from '../utils/response/apiResponse';
import { required, Required } from '../utils/response/responseHandlers';
import { WorkScheduleCreate, WorkScheduleUpdate } from './types';

const resourceName = 'workSchedules';
type ResourceName = typeof resourceName;

export class WorkSchedulesEndpoint extends BaseApi<ResourceName> {
  public readonly resourceName = resourceName;

  public create(data: WorkScheduleCreate): Required<ResourceName, Entity<ResourceName>[]> {
    const response = this._post<ResourceName>('create', data);
    return required(response);
  }

  public update(data: WorkScheduleUpdate): Required<ResourceName, Entity<ResourceName>[]> {
    const response = this._put<ResourceName>('update', data);
    return required(response);
  }
}
